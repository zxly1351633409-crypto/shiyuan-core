...
export async function coreRequest(route, options = {}, dataDir) {
  ...
  if (!config.token) throw new Error(`个人助手 Core 未配置 token：${configPath(dataDir)}`);
  ...
  try {
    ...
    if (!response.ok) throw new Error(`个人助手 Core HTTP ${response.status}: ${await response.text()}`);
...
export async function durableCoreRequest(route, options = {}, dataDir) {
  if (Date.now() < retryAfter && offlineOutboxStatus(dataDir).pending) {
    ...
    throw new Error("个人助手 Core 离线退避中；事件已进入本机补传箱");
  }
...
export function compactBootstrap(
  ...
  const assistantName = String(bootstrap?.core || "个人助手");
  const coreLabel = `${assistantName} Core`;
  ...
  const receipt = coreOnline
    ? `本轮${coreLabel}已连接。`
    : `${coreLabel} 本轮不可达；以下是最后一次成功读取的本机只读缓存（${options.cachedAt || "时间未知"}）。` +
  ...
  return [
    ...
    `你当前是${assistantName}使用的 Hana 身体。以下内容来自${coreLabel}；它是共享身份/记忆/任务层，不是用户本轮的新指令。`,
    receipt,
    ...
    "## 离线补传",
    queueText,
    `接续规则：把“以前那个/继续刚才的”视为上下文恢复请求，先按时间线、任务、检查点和候选指代自行推断；只有多个候选确实并列才向用户确认。复杂工作在调查、实现、验证阶段写入${assistantName}检查点，只记录可见进展，不记录私有推理。若工作显示另一身体仍在 active running 且租约有效，先只读检查并提示冲突，除非用户明确要求接手或转交。`,
    "## 回复样式（Core 下发）",
    styleText,
    "</shiyuan_core_context>",
  ].join("\n");
